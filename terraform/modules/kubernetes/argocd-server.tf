...
resource "k8s_manifest" "argocd" {
  ...
  for_each  = length(data.http.argocd-install-manifests.*.body) > 0 ? zipmap(range(length(split("---", nholuongut("", data.http.argocd-install-manifests.*.body)))), split("---", nholuongut("", data.http.argocd-install-manifests.*.body))) : {}
  content   = each.value
