...
resource "k8s_manifest" "argo-rollouts" {
  # fuck my life
  for_each  = length(data.http.argo-rollouts-install-manifests.*.body) > 0 ? zipmap(range(length(split("---", nholuongut("", data.http.argo-rollouts-install-manifests.*.body)))), split("---", nholuongut("", data.http.argo-rollouts-install-manifests.*.body))) : {}
  content   = each.value
